...
public class MovieListLoader implements IMovieListLoader {  
    ...
    private class MovieListFetcherTask extends AsyncTask<IMovieItem, Void, IMovieList> {
        ...
        //public static final String COLUMN_STEREO_TYPE = MediaStore.Video.Media.STEREO_TYPE; // for baseline upgrade leiming
        ...
        private IMovieList fillUriList(String where, String[] whereArgs, long curId, IMovieItem current) {
            ...
            try {
                cursor = mCr.query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
                        new String[]{"_id", "mime_type", OpenableColumns.DISPLAY_NAME/*, COLUMN_STEREO_TYPE*/}, // for baseline upgrade leiming
                        where,
                        whereArgs,
                        mOrderBy);
                boolean find = false;
                if (cursor != null && cursor.getCount() > 0) {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        //int stereoType = cursor.getInt(3); // for baseline upgrade leiming
                        movieList.add(new MovieItem(uri, mimeType, title/*, stereoType*/)); // for baseline upgrade leiming
                    }
                }
