...
//    private boolean mHasGotMetaData = false;//PR593460 wanwan.ye(porting by liqiang for PR595899)
    private boolean mHasGotPreparedCallBack = false;
    private int  mDuration = 1;
    //PR490956 add the StreamingType 20130724 begin
    private boolean mIsLocal = false;

    public void setStreamingType(boolean isLocal) {
        mIsLocal = isLocal;
    }
    //PR490956 add the StreamingType 20130724  end
    ...
    private final MediaPlayer.OnInfoListener mInfoListener = new MediaPlayer.OnInfoListener() {
        ...
        public boolean onInfo(final MediaPlayer mp, final int what, final int extra) {
            ...
            }*/ //PR593460 wanwan.ye(porting by liqiang for PR595899)
            return false;
        }
    ...
    private void doPreparedIfReady(final MediaPlayer mp) {
        ...
        if (mHasGotPreparedCallBack && /*mHasGotMetaData && */!mNeedWaitLayout) {//PR593460 wanwan.ye(porting by liqiang for PR595899)
            doPrepared(mp);
        }
    }
        ...
        mErrorListener = new MediaPlayer.OnErrorListener() {
            public boolean onError(final MediaPlayer mp, final int frameworkErr, final int implErr) {
                ...
                if (getWindowToken() != null) {
                    //modified by qjz for PR494819 begin
                    int messageId=com.android.internal.R.string.VideoView_error_text_unknown;
                    ...
                    try {
                        ...
                        Log.w(TAG,"Warning:"+ex.toString());
                        //add by qjz for PR495700:It will occur the force close prompt when play popup video begin
                        Toast.makeText(mContext, r.getString(messageId), Toast.LENGTH_LONG).show();
                        Intent intent = new Intent();
                        intent.setAction("com.jrdcom.action.CLOSE_POPUP_WINDOW");
                        mContext.startService(intent);
                        //add by qjz for PR495700:It will occur the force close prompt when play popup video end
                    }
                    //modified by qjz for PR494819 end
                }
                return true;
            }
        };
...
//    }
    ...
    protected void openVideo() {
        ...
        try {
            ...
            mErrorListener.onError(mMediaPlayer, MediaPlayer.MEDIA_ERROR_UNKNOWN, 0);
            return;
            // PR482867 Xiaowei.xu begin
        } catch(final IllegalStateException ex){
             Log.w(TAG, "Unable to open content: " + mUri, ex);
            mErrorListener.onError(mMediaPlayer, MediaPlayer.MEDIA_ERROR_UNKNOWN, 0);
            return;
         // PR482867 Xiaowei.xu end
    ...
    private void doPrepared(final MediaPlayer mp) {
        ...
        //PR490956:Popup video cannot go on play after unlock screen
        //modified by qjz 20130724
        if (mTargetState == STATE_PLAYING||(mIsLocal && mTargetState == STATE_PAUSED)) {
            start();
        }
        if (LOG) {
