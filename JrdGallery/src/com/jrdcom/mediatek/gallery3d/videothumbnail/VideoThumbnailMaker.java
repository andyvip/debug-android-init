...
public final class VideoThumbnailMaker {
    private static final String TAG = "VideoThumbnailMaker";
    ...
    //PR-677300-yibin.yang@tcl.com-Begin porting code from MTK 667865
    ...
    //PR-677300-yibin.yang@tcl.com-End
    ...
    private static String getDynamicThumbnailPath(String inputName) {
        ...
        if (i == -1) {
            res = new StringBuilder(".dthumb/").append(inputName.hashCode()).append(DYNAMIC_CACHE_FILE_POSTFIX);
        } else {
            // i-1 can be -1. risk?
            res = new StringBuilder(inputName.substring(0, i+1)).append(".dthumb/").append(inputName.hashCode())
    ...
    private static int doTranscode(LocalVideo video) {
        ...
        long transcodeId = sCurrentHandle.get();//PR-677300-yibin.yang@tcl.com
        ...
        //PR-677300-yibin.yang@tcl.com-Begin
        int result = VideoTranscode.transcodeAdv(transcodeId, video.filePath, video.dynamicThumbnailPath + ".tmp", (long) targetRect.width(),(long) targetRect.height(), startTime, endTime, 0, 10);
    ...
    private static synchronized void handleVideo(LocalVideo video) {//PR-677300-yibin.yang@tcl.com
    ...
    private static boolean needGenDynThumb(LocalVideo video) {
        ...
        if (srcRect.width() <= ENCODE_WIDTH || srcRect.height() <= ENCODE_HEIGHT) {
            ...
            video.thumbNailState = LocalVideo.THUMBNAIL_STATE_DYNAMIC_GENERATED_SUCCESS;
            video.dynamicThumbnailPath = inputPath;
            return false;
        } else {
            String outputPath;
            ...
            File dynThumbTmpFile = new File(outputPath + ".tmp");
            if (dynThumbFile.exists()/* || dynThumbTmpFile.exists()*/) {
                video.thumbNailState = LocalVideo.THUMBNAIL_STATE_DYNAMIC_GENERATED_SUCCESS;
