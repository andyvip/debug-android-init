...
public class TemplateItemView extends View {
    // ===========================================================
    // Constants
    // ===========================================================
...
//  private GestureDetector gestureDetector;
    ...
    private PuzzleFormatPath mTemplateLayoutReader = new PuzzleFormatPath();
    private Vector<Bitmap> m_vecTexture = null;
    private Point mDeltaPivot = null;
    private Point mUpPivot = null;
     ...
     */
    public TemplateItemView(Context context, int pItemIndex) {
        super(context);
...
//      gestureDetector = new GestureDetector(new GestureListener());
    ...
    public void setPuzzleTexture(Vector<Bitmap> list)
    {
        m_vecTexture=list;
    }
    // ===========================================================
...
//      mControlZoomBmp = BitmapFactory.decodeResource(getResources(), R.drawable.rotate);
//      mControlDelBmp = BitmapFactory.decodeResource(getResources(), R.drawable.delete);
        mTargetBmp = pBitmap;
...
//      mControlBmpHeight = mControlZoomBmp.getHeight();
        ...
        int frameWidth = (int) this.getContext().getResources().getDimension(R.dimen.puzzlefree_whiteframe_width);
        frameWidth = 5;

        mBoundRect = new Rect(0, 0, mTargetBmpWidth, mTargetBmpHeight);
        mBoundRect.inset(-frameWidth, -frameWidth);

        mBoundDrawable = (NinePatchDrawable) this.getContext().getResources().getDrawable(R.drawable.ic_white_shadow);
        mBoundDrawable.setBounds(mBoundRect);
        mDstRect = new RectF();
        mMatrix = new Matrix();
        mPrePivot = new Point(0, 0);
        mLastPivot = new Point(0, 0);
        mLastPoint = new Point(0, 0);
        mDeltaPivot = new Point(0, 0);
        mUpPivot = new Point(0,0);
...
//      
        //add by biao.luo begin for pr533387
        if (ActivityTemplate.getModel().getTemplateLayout().size() <= key) {
            return;
        }
        //add by biao.luo end
        TemplateLayoutItem lLayoutItem = ActivityTemplate.getModel().getTemplateLayout().getItem(key);
        path = new Path(lLayoutItem.getPath());
        Matrix matrix = new Matrix();
        matrix.setScale(TemplateTool.mPuzzleScale, TemplateTool.mPuzzleScaleY);
        path.transform(matrix);
        
        dstBitmap = Bitmap.createBitmap((int)(lLayoutItem.getWidth()*TemplateTool.mPuzzleScale),(int)(lLayoutItem.getHeight()*TemplateTool.mPuzzleScaleY), Bitmap.Config.ARGB_8888);
        ...
        if (null != mTargetBmp) {
            ...
            mCanvas.restore();
          //yaogang.hao for PR 539491
            mCanvas.drawBitmap(mTargetBmp, mMatrix, mPathPaint);
            canvas.drawBitmap(dstBitmap, 0,0, null);
        }
//      if (isSelected && this.mPhotoType != PuzzleConstant.STYLE_DECOPIC_PHOTO) {
//          drawFrame(canvas);
...
//      canvas.drawBitmap(mControlDelBmp, mDstPoint[0] - mControlBmpWidth / 2, mDstPoint[1] - mControlBmpHeight / 2,
//              mPaint);
//      if (BitmapUtil.isValid(mControlZoomBmp))
//      canvas.drawBitmap(mControlZoomBmp, mDstPoint[4] - mControlBmpWidth / 2, mDstPoint[5] - mControlBmpHeight / 2,
//              mPaint);
    }
...
//        mPrePivot.x = (int) mDstRect.left;
//        mPrePivot.y = (int) mDstRect.top;
//        mLastPivot.x = mPrePivot.x;
//        mLastPoint.y = mPrePivot.y;
    }
...
//                  return true;
                    ...
                    mLastDegree = computeDegree(new Point((int) event.getX(), (int) event.getY()), new Point(
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                isMoved = true;
                mScaleX = (event.getX(0)+event.getX(1))/2;
                mScaleY = (event.getY(0)+event.getY(1))/2;
                // 计算两手指间的距离
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(mode == BIGGER)
                {
                    if(0 != mDstRect.left && mScale == mScaleDefault){
...
//      startScaleAnimation();
    ...
    private void startScaleAnimation() {
        ScaleAnimation localScaleAnimation = new ScaleAnimation(1.0f / mScale, 1.0f, 1.0f / mScale, 1.0f, mDstPoint[8],
                mDstPoint[9]);
    ...
    private void setTranslate(float mDeltaX, float mDeltaY) {
        ...
        this.invalidate();
        this.clearAnimation();
        TranslateAnimation localTranslateAnimation = new TranslateAnimation(-mDeltaX, 0, -mDeltaY, 0);
    ...
    private void setScale(float scale, float degree) {
        ...
        invalidate();
        clearAnimation();
        ScaleAnimation localScaleAnimation = new ScaleAnimation(1.0f / scale, 1.0f, 1.0f / scale, 1.0f, mDstPoint[8],
                mDstPoint[9]);
        localScaleAnimation.setDuration(300L);
        RotateAnimation localRotateAnimation = new RotateAnimation(-degree, 0, mDstPoint[8], mDstPoint[9]);
    ...
    public void translate(int x, int y) {
    	//PR-549938 added by xiaowei.xu begin
		 if ( mPrePivot == null || mLastPoint == null || mDstRect == null) {
			 return ;
		 }
		 //PR-549938 added by xiaowei.xu end
        ...
        if(mode == NONE){
            ...
            if(mPrePivot.x > 0)
            {
                if(mPrePivot.y > 0){
                    ...
                    mDeltaY = -mPrePivot.y;
                    mPrePivot.y = 0;
                }else if (mTargetBmp.getHeight()*mScale+mPrePivot.y < dstBitmap.getHeight()) {
                    mDeltaX = -mPrePivot.x;
                    mDeltaY = -(mTargetBmp.getHeight()*mScale-dstBitmap.getHeight()+mPrePivot.y);
                    mPrePivot.y +=  mDeltaY;
                } else {
                    mDeltaX = -mPrePivot.x;
                    mDeltaY = mPrePivot.y - mLastPivot.y;
                ...
                mPrePivot.x = 0;
            } else if (mPrePivot.y > 0) {
                if(mPrePivot.x>0){
                    mDeltaX = -mPrePivot.x;
                    mDeltaY = -mPrePivot.y;
                    mPrePivot.x = 0;
                }else if (mTargetBmp.getWidth()*mScale+mPrePivot.x < dstBitmap.getWidth()) {
                    mDeltaX = -(mTargetBmp.getWidth()*mScale-dstBitmap.getWidth()+mPrePivot.x);
                    mDeltaY = -mPrePivot.y;
                    mPrePivot.x += mDeltaX;
                }else {
                    mDeltaX = mPrePivot.x-mLastPivot.x;
    ...
    public void rotate(MotionEvent event) {
        if (event.getPointerCount() == 2) {
            ...
            mPreDegree = computeDegree(new Point((int) event.getX(), (int) event.getY()), new Point(
                    (int) mDstPoint[8], (int) mDstPoint[9]));
        }
...
//            if ((mTargetBmp.getWidth() * mScale * mScaleValue < 45 * MyData.nDensity ||
//                    mTargetBmp.getHeight() * mScale * mScaleValue < 45 * MyData.nDensity)) {
//                return;
//            }
//        }
        ...
        if(mode == BIGGER){
            if(mScale * mScaleValue  <= mScaleDefault){
                mScaleValue = mScaleDefault/mScale;
            }
        }
    ...
    private int isOnZoomCtrolBmp(int x, int y) {
        ...
        Rect rect = new Rect((int) mDstPoint[4] - mControlBmpWidth, (int) mDstPoint[5] - mControlBmpHeight,
    ...
    private int isOnDelCtrolBmp(int x, int y) {
        ...
        Rect rect = new Rect((int) mDstPoint[0] - mControlBmpWidth / 2, (int) mDstPoint[1] - mControlBmpHeight / 2,
                (int) mDstPoint[0] + mControlBmpWidth / 2, (int) mDstPoint[1] + mControlBmpHeight / 2);
...
//      Log.e("getMatrixsInfo", " mLastPivoty = " + this.mLastPivot.y);
    ...
    public interface OnTouchesListener {
        public void onTouchBegan(TemplateItemView view);
    }


    public interface onClickRemoveViewListener {
        public void doRemoveViewAction(TemplateItemView view);
    }

    public interface onMtDoubleTapListenerListener {
        public void doDoubleTapAction(TemplateItemView view);
    }
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (isClickInTextSprite) {
                ...
                if (mDoubleTapListenerListener != null) {
                    mDoubleTapListenerListener.doDoubleTapAction(TemplateItemView.this);
                }
