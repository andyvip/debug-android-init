...
import com.jrdcom.android.gallery3d.ui.TileImageView;
...
import com.jrdcom.android.gallery3d.util.RangeArray;
...
public class PhotoView extends GLView {
    ...
    class FullPicture implements Picture {
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            mImageLoadingState = mLoadingState;//PR658460-taoli-001
    ...
    private int mImageLoadingState = Model.LOADING_INIT;
    public int getLoadImageState(){
        return mImageLoadingState;
    }
    ...
    private class MyGestureListener implements GestureRecognizer.Listener {
        ...
        public boolean onDoubleTap(float x, float y) {
            ...
            if (scale <= .75f || controller.isAtMinimalScale()) {
                if (controller.getImageWidth() * controller.getImageHeight() >= 13000000) {
                    controller.zoomIn(x, y,
                            Math.max(PositionController.SCALE_LIMIT_ABOVE_1300W, Math.min(scale
                                    * PositionController.SCALE_LIMIT_ABOVE_1300W, 4.0f)));
                } else if (controller.getImageWidth() * controller.getImageHeight() >= 8000000) {
                    controller.zoomIn(x, y,
                            Math.max(PositionController.SCALE_LIMIT_ABOVE_800W, Math.min(scale
                                    * PositionController.SCALE_LIMIT_ABOVE_800W, 4.0f)));
                } else if (controller.getImageWidth() * controller.getImageHeight() >= 2000000) {
                    controller.zoomIn(x, y,
                            Math.max(PositionController.SCALE_LIMIT_ABOVE_200W, Math.min(scale
                                    * PositionController.SCALE_LIMIT_ABOVE_200W, 4.0f)));
                } else {
                    controller.zoomIn(x, y,
                            Math.max(PositionController.SCALE_LIMIT_BELOW_200W, Math.min(scale
                                    * PositionController.SCALE_LIMIT_BELOW_200W, 4.0f)));
                }
                // CR 462613 jipu.xiong@tcl.com 2013-06-03 begin
//                controller.zoomIn(x, y, Math.max(1.25f, Math.min(scale * 1.25f, 4.0f)));
                // CR 462613 jipu.xiong@tcl.com 2013-06-03 end
            } else {
        ...
        @Override
        public boolean onScroll(float dx, float dy, float totalX, float totalY, int zoom) {
...
//        }
    ...
    private boolean switchWithCaptureAnimationLocked(int offset) {
        ...
        if (offset == 1) {
            ...
            return true;//add by qjz for camera photo fullScreen display 20130510
        } else if (offset == -1) {
            if (mPrevBound >= 0) return false;
    ...
    public void leaveCameraPreview() {
        ...
        // PR 476969 jipu.xiong@tcl.com 2013-06-24 begin
        if (mListener != null) {
            // PR 490379 jipu.xiong@tcl.com begin
            mListener.onFullScreenChanged(false, Listener.FULLSCREEN_TYPE_MINIMAL_SCALE);
            // PR 490379 jipu.xiong@tcl.com end
            mListener.onLeaveCamera();
        }
        // PR 476969 jipu.xiong@tcl.com 2013-06-24 end
