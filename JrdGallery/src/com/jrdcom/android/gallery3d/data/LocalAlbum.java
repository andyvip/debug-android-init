...
//    private static final int INVALID_COUNT = -1;
    private static final int INVALID_COUNT = 0;//add by biao.luo for pr503218
    ...
    //add for drm wifi display
    private String getDrmWhereClause(){
        if(mIsDrmSupported && !DrmHelper.getDrmFlag() && mIsImage){
            return "("+DrmHelper.getDrmWhereClause()+") AND ";
        }
        else{
            return "";
        }
    ...
    public boolean isCameraRoll() {
        //PR929582 when default storage has been changed, we should refresh bucked id by fengke at 2015.02.12 start
        //get default storage path in run time
    ...
    public ArrayList<MediaItem> getMediaItem(int start, int count) {
        ...
        //huanglin 20130914 for PR516174 begin
        Cursor cursor;
        try {
            cursor = mResolver.query(
                //PR504333-Wentao-Wan-001 begin
                //uri, mProjection, mWhereClause,
                uri, mProjection, getDrmWhereClause() + mWhereClause,
                //PR504333-Wentao-Wan-001 end
                mWhereClauseArgs, //new String[]{String.valueOf(mBucketId)},
                mOrderClause);
        } catch(Exception e){
        ...
        //huanglin 20130914 for PR516174 end
        if (cursor == null) {
    ...
    public int getMediaItemCount() {
        if (mCachedCount == INVALID_COUNT) {
            //add by qjz for PR482321 20130703 begin
            Cursor cursor;
            try {
                cursor = mResolver.query(
                    //PR504333-Wentao-Wan-001 begin
                    //mBaseUri, COUNT_PROJECTION, mWhereClause,
                    mBaseUri, COUNT_PROJECTION, getDrmWhereClause() + mWhereClause,
                    //PR504333-Wentao-Wan-001 end
                    mWhereClauseArgs,//new String[]{String.valueOf(mBucketId)}, 
                    null);
            } catch (Exception ex) {
            ...
            //add by qjz for PR482321 20130703 end
            if (cursor == null) {
    ...
    public String getName() {
         //PR951891 After change language ,the language of camera album is not changed by limin.zhuo motified at 2015.03.30 start
        return getLocalizedName( mApplication.getResources(), mBucketId, mName);  
        //PR951891 After change language ,the language of camera album is not changed by limin.zhuo motified at 2015.03.30 end
    }
