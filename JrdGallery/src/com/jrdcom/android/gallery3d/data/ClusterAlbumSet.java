...
public class ClusterAlbumSet extends MediaSet implements ContentListener {
    ...
    public static int currentIndexOfSet;
    ...
    //PR861016 modify monkey test error in ClusterAlbumSet updateClustersContents by fengke at 2014.12.03 start
        ...
        try {
            ...
            for (int i = n - 1; i >= 0; i--) {
              ...
              //  Log.d(TAG, "oldPaths.size==="+oldPaths.size());
                ...
                //PR861016 modify monkey test error in ClusterAlbumSet updateClustersContents by fengke at 2014.12.03 start
                if (i > mAlbums.size() -1) {
                    Log.w(TAG, " fengke mAlbums.size()1=" + mAlbums.size() + ", i = "+ i + ", error return do noting");
                    return;
                }
                //PR861016 modify monkey test error in ClusterAlbumSet updateClustersContents by fengke at 2014.12.03 end
            ...
            if ((existing.size() + deletedItem > oldPathHashSet.size())) {
                Log.d(TAG, " offsetOFStack==" + offsetInStack + " currentIndexOfSet="
                        + currentIndexOfSet);
                if (offsetInStack >= 1) {
            ...
            Log.w(TAG," fengke updateClustersContents IndexOutOfBoundsException:" + e);
        }
    }
    //PR861016 modify monkey test error in ClusterAlbumSet updateClustersContents by fengke at 2014.12.03 end
                ...
                try {
                    ...
                    for (int j = 0; j < sizeofAddPaths; j++) {
                        ...
                        for (; k < sizeOfOldMediaItems; k++) {
                            //yaogang.hao for PR 487420 Pop up force close when selected all photos to delete.
                            if (currentIndexOfSet<=mAlbums.size()-1 && k<=mOldItem.size()-1
                                    && item.getDateInMs() == mOldItem.get(k).getDateInMs()) 
                            {
                        ...
                        // yaogang.hao for PR528183 currentIndexOfSet can not -1
                        if (k == sizeOfOldMediaItems && currentIndexOfSet > 0) {
