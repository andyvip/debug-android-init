...
import com.jrdcom.android.gallery3d.app.PanoramaMetadataSupport;
import com.jrdcom.android.gallery3d.app.StitchingProgressManager;
...
import com.jrdcom.android.gallery3d.util.UpdateHelper;
...
import com.jrdcom.mediatek.gallery3d.data.RequestHelper;
...
public class LocalImage extends LocalMediaItem {
    ...
    //private static final int INDEX_MPO_SUB_TYPE = 16; // for baseline upgrade leiming
    // private static final int INDEX_STEREO_TYPE = 17; // for baseline upgrade leiming
    private static final int INDEX_CAPTURE_MODE = 16; // for baseline upgrade leiming
    ...
    static final String[] PROJECTION =  {
            ...
            //added to support Stereo display
            //Images.Media.MPO_TYPE,	//16  // for baseline upgrade leiming
            //Video.Media.STEREO_TYPE,	//17 // for baseline upgrade leiming
            "capture_mode",            //16
    };
    ...
    private void loadFromCursor(Cursor cursor) {
        ...
        captureMode = cursor.getString(INDEX_CAPTURE_MODE);
//captureMode = "normal";
       ...
       // mMpoSubType = cursor.getInt(INDEX_MPO_SUB_TYPE); // for baseline upgrade leiming
        // M: added for stereo display
       // stereoType = cursor.getInt(INDEX_STEREO_TYPE); // for baseline upgrade leiming
    }
    ...
    protected boolean updateFromCursor(Cursor cursor) {
        ...
        captureMode = uh.update(captureMode, cursor.getString(INDEX_CAPTURE_MODE));
//captureMode = "normal";
        if (!mIsStereoDisplaySupported ||
      ...
      //  mMpoSubType = cursor.getInt(INDEX_MPO_SUB_TYPE); // for baseline upgrade leiming
        // M: added for stereo display
       // stereoType = uh.update(stereoType, cursor.getInt(INDEX_STEREO_TYPE)); // for baseline upgrade leiming
        return uh.isUpdated();
    }
    ...
    public static class LocalImageRequest extends ImageCacheRequest {
        ...
        LocalImageRequest(GalleryApp application, Path path, int type,
            ...
            android.util.Log.i("wp", " mLocalFilePath = " +  mLocalFilePath);
        }
        ...
        public Bitmap onDecodeOriginal(JobContext jc, final int type) {
            ...
            android.util.Log.i("wp", "bitmap = " + bitmap  );
    ...
    public int getSupportedOperations() {
        //FR590978-Zonghua-Jin for pr485699 begin
        if(MediatekFeature.isDrmSupported() && isDrm()) {
            return getDrmSupportedOperations();
        }
        //FR590978-Zonghua-Jin for pr485699 end
    ...
    public int getDrmSupportedOperations() {
        ...
        operation = modifyForImageType(operation);//PR405318-xiaobin-yang-001
        ...
        file_path=filePath;
        DrmManagerClient drmClient = DrmManagerClient.getInstance(mApplication.getAndroidContext());
        if ((DrmStore.RightsStatus.RIGHTS_VALID == drmClient.checkRightsStatus(filePath, DrmStore.Action.DISPLAY))
                && !(drmClient.hasCountConstraint(filePath))) {
...
//            initMpoSubType();
        ...
        if (mIsDrmSupported && isDrm() && OmaDrmStore.DrmMethod.METHOD_FL != drm_method) {//PR511555-xiaobin-yang-001
            if (!hasDrmRights()) {
        ...
        } else if (OmaDrmStore.DrmMethod.METHOD_FL == drm_method) {
            if (!hasDrmRights()) {
                subType |= SUBTYPE_DRM_NO_RIGHT;
            } else {
                subType |= SUBTYPE_DRM_HAS_RIGHT;
