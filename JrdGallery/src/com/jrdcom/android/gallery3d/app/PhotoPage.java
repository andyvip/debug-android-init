...
import com.jrdcom.android.gallery3d.app.PhotoPage;
import com.jrdcom.android.gallery3d.app.SinglePhotoDataAdapter;
import com.android.jrdcamera.Camera;
import com.jrdcom.android.gallery3d.app.Gallery;
...
import com.jrdcom.android.gallery3d.data.ComboAlbum;
import com.jrdcom.android.gallery3d.data.DataManager;
import com.jrdcom.android.gallery3d.data.FilterDeleteSet;
import com.jrdcom.android.gallery3d.data.FilterSource;
import com.jrdcom.android.gallery3d.data.LocalImage;
import com.jrdcom.android.gallery3d.data.MediaDetails;
...
import com.jrdcom.android.gallery3d.data.MtpSource;
import com.jrdcom.android.gallery3d.data.Path;
import com.jrdcom.android.gallery3d.data.SecureAlbum;
import com.jrdcom.android.gallery3d.data.SecureSource;
import com.jrdcom.android.gallery3d.data.SnailAlbum;
import com.jrdcom.android.gallery3d.data.SnailItem;
import com.jrdcom.android.gallery3d.data.SnailSource;
...
import com.jrdcom.android.gallery3d.ui.DetailsHelper;
import com.jrdcom.android.gallery3d.ui.DetailsHelper.CloseListener;
import com.jrdcom.android.gallery3d.ui.DetailsHelper.DetailsSource;
import com.jrdcom.android.gallery3d.ui.GLCanvas;
import com.jrdcom.android.gallery3d.ui.GLRoot;
import com.jrdcom.android.gallery3d.ui.GLRoot.OnGLIdleListener;
import com.jrdcom.android.gallery3d.ui.GestureRecognizer.Listener;
import com.jrdcom.android.gallery3d.ui.GLView;
import com.jrdcom.android.gallery3d.ui.ImportCompleteListener;
import com.jrdcom.android.gallery3d.ui.MenuExecutor;
import com.jrdcom.android.gallery3d.ui.OneFingerDetectorTips;//PR912344 Add One finger zoom function by fengke at 2015.01.26
import com.jrdcom.android.gallery3d.ui.PhotoView;
...
import com.jrdcom.android.gallery3d.data.LocalImage;
import com.jrdcom.android.gallery3d.data.LocalVideo;
...
public class PhotoPage extends ActivityState implements
    ...
    private final boolean  DEFINE_OPEN_BEAUTY = false;
    ...
    private static final int MSG_BACK_TO_FIRST = 18;
    ...
    private static final int MSG_LAUNCHE_SLIDE=133;
    ...
    //add by qjz for PR449035 begin
    private PopupWindow mPopupWindow;
    //private ImageView mPopupPosition;
    private View mPopupWindowlayout;
    //add by qjz for PR449035 end

    // CR 462613 jipu.xiong@tcl.com begin
    private TextView captureModeView;
    private String mFBCaptureModeHint = "";
    private String mHDRCaptureModeHint = "";
    private String mCaptureModeCurrentPhoto;
    // CR 462613 jipu.xiong@tcl.com end

    //PR465439-lilei-begin
    private TextView mPrint;
    //PR465439-lilei-end
    private TextView mProtectInfo;//PR498347-xiaobin-yang-001
    private OneFingerDetectorTips tips;//PR912344 Add One finger zoom function by fengke at 2015.01.26 start
    ...
    public void onCreate(Bundle data, Bundle restoreState) {
        ...
        if (OneFingerDetectorTips.isFirstTime(mActivity.getAndroidContext())) {
            tips = new OneFingerDetectorTips(mActivity.getAndroidContext());
            //PR605155,v-nj-feiqiang.cheng add begin fengke porting to soul4.5 2014.02.28
			if (mActivity.getComponentName().getClassName()
					.equals("com.android.jrdcamera.CameraLauncher")
					|| mActivity.getComponentName().getClassName()
							.equals("com.android.jrdcamera.Camera")) {
				tips.setTipsContent(R.string.onef_inger_zoomer_tips_camera);
			}
		    //PR605155,v-nj-feiqiang.cheng add end fengke porting to soul4.5 2014.02.28
        }
        ...
        initCaptureModeHint(); // CR 462613 jipu.xiong@tcl.com
            ...
            public void handleMessage(Message message) {
                switch (message.what) {
                    ...
                    case MSG_ON_CAMERA_CENTER: {
                        ...
                        if (stayedOnCamera) {
                            if (mAppBridge == null) {
                                //modified  by yaogang.hao 
                                //pressed 2 times back to gallery
                                //mPhotoView.switchToImage(1);

                                launchCamera();
                                mHandler.sendEmptyMessageDelayed(MSG_BACK_TO_FIRST, 50);
                                //mPhotoView.switchToImage(1);
                                //no images situation back to gallery
        ...
        if (mSetPathString != null) {
            ...
            if (mAppBridge != null) {
                ...
                //add a button on the Gallery when lunch from camera.
                RelativeLayout camera_root = (RelativeLayout) ((Activity) mActivity)// change by jiankun.liu pano Photo
                        .findViewById(R.id.camera_root);
                if (camera_root != null) {
                    mBottomControls = new PhotoPageBottomControls(this, mActivity, camera_root);
                }
                ...
                if (guestMode == 1) {
                    mSetPathString = "/filter/empty/{/local/all/0}";
                }
            ...
            pda.setDataListener(new PhotoDataAdapter.DataListener() {
                ...
                public void onPhotoChanged(int index, Path item) {
                    ...
                    //PR511387-lilei-begin
                    //PR506354-lilei-begin
                    if(mIsActive && mMediaSet.getMediaItemCount() <= 1 && mAppBridge == null
        ...
        //add by qjz for PR449035 begin
        //mPopupPosition = mActivity.getPositionImage();
        mPopupWindowlayout = View.inflate(mActivity.getAndroidContext(), R.layout.photo_pop_window, null);
        //PR465439-lilei-begin
        //TextView print = (TextView) mPopupWindowlayout.findViewById(R.id.item_print);
        mPrint = (TextView) mPopupWindowlayout.findViewById(R.id.item_print);
        ...
        mPrint.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                mMenuExecutor.onMenuClicked(R.id.action_print, mConfirmDialogListener,false,true);
            }
        });
        //PR465439-lilei-end
        TextView details = (TextView) mPopupWindowlayout.findViewById(R.id.item_details);
        ...
        //PR498347-xiaobin-yang-001 begin
        mProtectInfo = (TextView) mPopupWindowlayout.findViewById(R.id.item_protect_info);
        mProtectInfo.setVisibility(View.GONE);
        mProtectInfo.setText(com.mediatek.internal.R.string.drm_protectioninfo_title);
        mProtectInfo.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                // show drm protect info
                mPopupWindow.dismiss();
                MediaItem current = mModel.getMediaItem(0);
                if (current == null) {
                    return;
                ...
                OmaDrmClient drmManagerClient = OmaDrmClient.newInstance(mActivity);
                if (null != drmManagerClient) {
                	OmaDrmUiUtils.showProtectionInfoDialog(
                            (Activity) mActivity, mActivity.getDataManager()
                                    .getContentUri(current.getPath()));
                } else {
                    Log.e(TAG, "onItemSelected:get drm manager client failed!");
                }
            }
        });
        //PR498347-xiaobin-yang-001 end
        //add by qjz for PR449035 end
    ...
    public boolean canDisplayBottomControl(int control) {
        ...
        switch(control) {
            //FR418632 add by yashuang.mu@tcl.com for Bottom Menu begin
            case R.id.photopage_bottom_controls_menu:
                // PR 449148 add by yashuang.mu@tcl.com for Bottom Menu show begin.
                if (mCurrentPhoto.getMediaType() == MediaObject.MEDIA_TYPE_IMAGE
                        || mCurrentPhoto.getMediaType() == MediaObject.MEDIA_TYPE_VIDEO// yaogang.hao
                                                                                       // for
                                                                                       // PR466523
                ) {
                    //add by biao.luo for pr514994 begin
                    LinearLayout controls_menu_Lay = (LinearLayout) ((Activity) mActivity)
                            .findViewById(R.id.photopage_bottom_controls_menu);
                    controls_menu_Lay.setClickable(false);
                    boolean support_share = ((mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_SHARE) != 0);
                    boolean support_setas = ((mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_SETAS) != 0);
                    boolean support_edit = (mMediaSet != null && (mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_EDIT) != 0);
                    boolean support_slide = canDoSlideShow();
                    ImageButton shareButton = (ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_share);
                    shareButton.setEnabled(support_share);
                    shareButton.setImageResource(support_share ? R.drawable.photopage_control_share : R.drawable.photopage_control_share_gray);
                    
                    ImageButton setasButton = (ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_setas);
                    setasButton.setEnabled(support_setas);
                    setasButton.setImageResource(support_setas ? R.drawable.photopage_control_set_as : R.drawable.photopage_control_set_as_gray);
                    
                    ImageButton editButton = (ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_edit);
                    editButton.setEnabled(support_edit);
                    editButton.setImageResource(support_edit ? R.drawable.photopage_control_edit : R.drawable.photopage_control_edit_gray);
                    
                    ImageButton slideButton = (ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_slide);
                    slideButton.setEnabled(support_slide);
                    slideButton.setImageResource(support_slide ? R.drawable.photopage_control_slideshow : R.drawable.photopage_control_slideshow_gray);
//                    if ((mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_SHARE) == 0) {
//                        controls_menu_Lay.findViewById(R.id.photopage_bottom_control_share)
//                                .setEnabled(false);
//                        //add by biao.luo for pr510249 
//                        ((ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_share)).setImageResource(R.drawable.photopage_control_share_gray);
//                    }
//                    if ((mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_SETAS) == 0) {
//                        controls_menu_Lay.findViewById(R.id.photopage_bottom_control_setas)
//                                .setEnabled(false);
//                        //add by biao.luo for pr510249 
//                        ((ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_setas)).setImageResource(R.drawable.photopage_control_set_as_gray);
//                    }
//                    if (mMediaSet == null
//                            || (mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_EDIT) == 0) {
//                        controls_menu_Lay.findViewById(R.id.photopage_bottom_control_edit)
//                                .setEnabled(false);
//                        //add by biao.luo for pr510249 
//                        ((ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_edit)).setImageResource(R.drawable.photopage_control_edit_gray);
//                    }
//                    if (!canDoSlideShow()) {
//                        controls_menu_Lay.findViewById(R.id.photopage_bottom_control_slide)
//                                .setEnabled(false);
//                        //add by biao.luo for pr510249 
//                        ((ImageButton)controls_menu_Lay.findViewById(R.id.photopage_bottom_control_slide)).setImageResource(R.drawable.photopage_control_slideshow_gray);
//                    }
                    //add by biao.luo end
                }
                // PR 449148 add by yashuang.mu@tcl.com for Bottom Menu show end.

            //FR418632 add by yashuang.mu@tcl.com for Bottom Menu end
            //yaogang.hao yaogang.hao for PR466523
            case R.id.photopage_bottom_control_edit:
                return mHaveImageEditor && mShowBars
                        && !mPhotoView.getFilmMode()
                        // && (mCurrentPhoto.getSupportedOperations() &
                        // MediaItem.SUPPORT_EDIT) != 0
                        // PR485699-xiaobin-yang-001
    ...
    public void onMenuBottomControlClicked(int control) {
        switch (control) {
            case R.id.photopage_bottom_control_share:
                ...
                return;
            case R.id.photopage_bottom_control_edit:
                if(DEFINE_OPEN_BEAUTY)//Beauty function
                {
                    launchPhotoEditorBeauty();
                }
                else 
                {
                    launchPhotoEditor();
            ...
            case R.id.photopage_bottom_control_setas:
                //PR658460-taoli-begin 001
                if (mPhotoView != null && mPhotoView.getLoadImageState() == Model.LOADING_FAIL) {
                    Toast.makeText(mActivity, mActivity.getResources().getString(R.string.cannot_load_image), Toast.LENGTH_SHORT).show();
                    return;
                }
                //PR658460-taoli-end 001
                launchPhotoSetas();
                return;
            case R.id.photopage_bottom_control_slide:
                // launchPhotoSlide();
                mHandler.obtainMessage(MSG_LAUNCHE_SLIDE);
                mHandler.sendEmptyMessage(MSG_LAUNCHE_SLIDE);
                return;
            default:
                return;
        }
    ...
    /**
     * @author yaogang.hao
     * function:Enter into phone beauty
     */
    public void launchPhotoEditorBeauty()
    {
        MediaItem current = mModel.getMediaItem(0);
...
//            }, mActivity);
    ...
    private void launchPhotoShare() {
        ...
        //PR448635 add by yashuang.nu@tcl.com 2013,May,04 begin
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_SEND);
        intent.setType(current.getMimeType());
        intent.putExtra(Intent.EXTRA_STREAM, current.getContentUri());
        Log.d(TAG, "Share Uri file: " + current.getContentUri());
        Log.d(TAG, "Share file mimetype: " + current.getMimeType());
        // xiaodaijun PR653237 start
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK |Intent.FLAG_ACTIVITY_CLEAR_TOP);
        // xiaodaijun PR653237 end
      ...
      //PR448635 add by yashuang.nu@tcl.com 2013,May,04 end
...
//        Intent intent = new Intent(Intent.ACTION_ATTACH_DATA);
        ...
        if (DrmManagerClient.isMTKDrmEnable()) {
            if (current.isDrm()) {
                intent = new Intent("android.drm.action.ATTACH_DATA");
            } else {
...
//        add for PR 631865 by GuiChen end
    ...
    private void launchPhotoSlide() {
        ...
        //PR630526-taoli-begin 001
        ActivityState mActivityState = mActivity.getStateManager().getTopState();
        if (mActivityState != this) {
            return;
        }
        mActivityState = null;
        //PR630526-taoli-end 001
...
//        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK|Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    private void updateMenuOperations() {
        ...
        if (MediaObject.MEDIA_TYPE_IMAGE == mCurrentPhoto.getMediaType()) {
            ...
            // FR418632 add by yashuang.mu@tcl.com for hide Share and edit begin
            supportedOperations &= ~MediaObject.SUPPORT_SHARE;
            supportedOperations &= ~MediaObject.SUPPORT_EDIT;
            // FR418632 add by yashuang.mu@tcl.com for hide Share and Edit end
    ...
    private void showBars() {
        ...
        mOrientationManager.unlockOrientation();
        mActionBar.show();
        showCaptureModeHint(); // CR 462613 jipu.xiong@tcl.com
    ...
    private void hideBars() {
        ...
        mShowBars = false;
        mActionBar.hide();
        hideCaptureModeHint(); // CR 462613 jipu.xiong@tcl.com
    ...
    private void onUpPressed() {
        ...
        if (mAppBridge == null) {
            ...
            Intent intent = null;
            //add by biao.luo for PR511984 begin
            boolean isCameraRollEmpty = true;
            if (mMediaSet != null) {
                int num = mMediaSet.getMediaItemCount();
                if (num > 2) {
                    isCameraRollEmpty = false;
                } else if (num == 2) {
                    MediaItem item = mMediaSet.getMediaItem(0, 2).get(1);
                    if (item.getMediaType() == MediaObject.MEDIA_TYPE_IMAGE ||
                            item.getMediaType() == MediaObject.MEDIA_TYPE_VIDEO)
                        isCameraRollEmpty = false;
                }
            }
            //add by biao.luo for PR511984 end
            //M: for OP2, return to the top of gallery when press back key
            if (MediatekFeature.preferGoTopWhenBack() || isCameraRollEmpty) { //add by biao.luo for PR511984 
                intent = new Intent(mActivity.getAndroidContext(), Gallery.class);
            } else {
                // xiaodaijun PR660331 start
                //intent = new Intent(Intent.ACTION_VIEW);
                intent = new Intent("com.jrdcom.action.VIEW");
                // xiaodaijun PR660331 end
    ...
    protected boolean onCreateActionBar(Menu menu) {
        ...
        mActionBar.createActionBarMenu(R.menu.new_photo, menu);
        //PR486999 Xiaowei.Xu begin
        if(menu.findItem(R.id.action_delete).isVisible()){
        ...
        switch (action) {
            ...
            case R.id.action_more:
                if (mPopupWindow == null) {
                    mPopupWindow = new PopupWindow(mPopupWindowlayout, LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                ...
                if (!mPopupWindow.isShowing()) {
                    ...
                    mPopupWindow.showAtLocation(view, 0, view.getWidth(),mActivity.getActionBar().getHeight()+view.getTop());
                    //mPopupWindow.showAsDropDown(mPopupPosition, -1, -1);
                } else {
                    mPopupWindow.dismiss();
                }
                //PR498347-xiaobin-yang-001 begin
                if ((mModel.getMediaItem(0).getSupportedOperations() & MediaObject.SUPPORT_DRM_INFO) == 0){
                    mProtectInfo.setVisibility(View.GONE);
                } else {
    ...
    public void onSingleTapUp(int x, int y) {
        ...
        if (playVideo) {
            ...
            if (captureModeView == null) {
                captureModeView = (TextView) ((Activity) mActivity).findViewById(R.id.capture_mode);
            }
            // CR 462613 jipu.xiong@tcl.com end
            toggleBars();
        }
    ...
    public boolean tryConsumeDrmRights(MediaItem item) {
        ...
        if (mIsDrmSupported && !mModel.enteredConsumeMode() && 
            ...
            return true;
          //fix PR 631861 by guichen begin
        } else if (mIsDrmSupported && item.isDrm() && item.isDrmMethod(OmaDrmStore.DrmMethod.METHOD_FL)) {
            //consume
            mModel.enterConsumeMode();
            //hide bar
            hideBars();
            return true;
            //fix PR 631861 by guichen end
        }
        return false;
    }
...
//            mActivity.getGLRoot().setStereoMode(false, true);
        ...
        hideCaptureModeHint();// PR476812 ming.zhang modify
    }
...
Log.i(TAG,"showStereoShareDialog:start new intent!");
    ...
    private void updateActionBarTitle() {
        ...
        updateCaputreModeHint(); // CR 462613 jipu.xiong@tcl.com
    }

    // CR 462613 jipu.xiong@tcl.com 2013-06-04 begin
    /*
     * show the special camera mode info
     */
    private void initCaptureModeHint() {
        captureModeView = (TextView) ((Activity) mActivity).findViewById(R.id.capture_mode);// PR476812 ming.zhang modify
        mHDRCaptureModeHint = mActivity.getResources().getString(
                com.android.jrdcamera.R.string.pref_camera_capturemode_enrty_hdr);
        mFBCaptureModeHint = mActivity.getResources().getString(
                com.android.jrdcamera.R.string.pref_camera_capturemode_enrty_fb);
    }

    private void updateCaputreModeHint() {
        if (mCurrentPhoto != null && captureModeView != null) {
            mCaptureModeCurrentPhoto = mCurrentPhoto.getCaptureMode();
            if ("hdr".equals(mCaptureModeCurrentPhoto)) {
                captureModeView.setText(mHDRCaptureModeHint);
                updateCaptureModeVisible();
            } else if ("face_beauty".equals(mCaptureModeCurrentPhoto)) {
                captureModeView.setText(mFBCaptureModeHint);
                updateCaptureModeVisible();
            } else {
    ...
    private void updateCaptureModeVisible() {
        if (mShowBars) {
    ...
    private void hideCaptureModeHint() {
        if (captureModeView == null) {
            return;
        }
        captureModeView.setVisibility(View.GONE);
    ...
    private void showCaptureModeHint() {
        if (captureModeView == null) {
            return;
        }
        updateCaputreModeHint();
    }

    // CR 462613 jipu.xiong@tcl.com 2013-06-04 end
    /// M: {@
            ...
            if (totalCount != 0) {
                ...
                if (index >= 0 && index < totalCount) {
                    if (mLastIndex == 0xFFFF || mLastIndex != index) {
                        MtkLog.d(TAG, "show frame by gyro sensor, [NO. " + index + "]");
