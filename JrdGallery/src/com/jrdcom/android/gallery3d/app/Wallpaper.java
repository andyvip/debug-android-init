    ...
    protected void onResume() {
        ...
        switch (mState) {
            case STATE_INIT: {
                ...
                if (mPickedItem == null) {
                    ...
                    if (MediatekFeature.isDrmSupported()) {
                        request.putExtra(OmaDrmStore.DrmExtra.EXTRA_DRM_LEVEL,
            ...
            case STATE_PHOTO_PICKED: {
                ...
                //add for PR 658251 by guichen begin
                String action = CropImage.ACTION_CROP;
                //PR690761, tian.shi, by adding startsWith(...), do not go thru this until it is in media store.
                if (mPickedItem != null && mPickedItem.toString().startsWith("content://media/") && DrmManagerClient.isMTKDrmEnable()) {
                    try {
                        DrmManagerClient drmManager = DrmManagerClient
                                .getInstance(getApplicationContext());
                        if (drmManager.isDrm(mPickedItem)) {
                            action = "com.android.drm.action.CROP";
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
...
//                Intent request = new Intent(CropImage.ACTION_CROP)
                ...
                //add for PR 658251 by guichen end
                        ...
                        .setPackage("com.jrdcom.android.gallery3d");//PR689509-taoli-001
